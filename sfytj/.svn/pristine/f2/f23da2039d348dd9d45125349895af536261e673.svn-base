...
!function (a, b) {
    ...
    a.Detect = function () {
        ...
        function d() {
            ...
            if (a.GetUserMedia && (b.getUserMedia = !0), a.PeerConnectioin && (b.peerConnection = !0), b.peerConnection) {
                var c = new a.PeerConnectioin({iceServers: [{url: "stun:api.shishimao.com:3478"}]});
        ...
        var f = this, g = 1, h = 2, i = 3, j = 4, k = 5, l = 6, m = 7, n = 8, o = 9, p = 10, q = b(), r = c(), s = d(), t = [h, k, l, m, n, p, j];
            ...
            try {
                ...
                MediaStreamTrack.getSources(function (c) {
                    for (var d = 0; d < c.length; d++) {
                        var e = c[d];
                        b.push({type: e.kind, label: e.label, id: e.id})
                    }
                    a(null, b)
                })
            } catch (c) {
    ...
    a.Errors = function (a, b, c) {
        var d = ["Stream accepts Denied.", "Set RTCat.extensionId before get ScreenSharing.", "Unknown Container!", "The website does not match the extension."], e = ["Sending a file once time", "Can not send a message while Channel is close!", "Can not send a file while Channel is close!"], f = ["Session has connected", "The type you send is not supported", "Too long for sending"], g = [d, e, f];
    ...
    a.EventEmitter = function () {
        ...
        return a.events = {}, a.onceEvents = {}, a.on = function (b, c) {
            ...
            var d, e, f = a.onceEvents[b], g = Array.prototype.slice.call(arguments, 1);
            if (f) {
    ...
    var a = a || {};
    a.Receiver = function (b, c) {
        function d() {
            function c(b) {
                r.stream = new a.Stream({
                    stream: b.stream,
                    local: !1
                }), f(i), r.emit("stream", r.stream), r.emit("debug_log", {eventName: "receive stream"})
            }

            function d(a) {
                r.channel = a.channel, e(r.channel), r.emit("debug_log", {eventName: "connect datachannel"})
            }

            function g(a) {
                if (a.candidate) {
                    var data = {};
                    data.sdpMid = a.candidate.sdpMid;
                    data.sdpMLineIndex = a.candidate.sdpMLineIndex;
                    data.candidate = a.candidate.candidate;
                    var b = {to: o, channelId: n, candidate: data, type: "answer"};
            ...
            function h() {
                i.createAnswer(function (b) {
                    i.setLocalDescription(new a.SessionDescription(b), function () {
                        ...
                        data.type = b.type;
                        data.sdp = b.sdp;
                        var c = {to: o, sdp: data, channelId: n};
            ...
            b.stream && (i.onaddstream = c), b.data && (i.ondatachannel = d), i.oniceconnectionstatechange = function (a) {
                ...
                i.addIceCandidate(a), r.emit("debug_log", {eventName: "add candidate"})
            }, r.close = function () {
        ...
        function e(a) {
            a.onopen = function () {
                ...
                if ("message" === b.type)r.emit("message", b.content); else if ("video" === b.type); else if ("file" === b.type && (q[b.filename] = q[b.filename] || [], q[b.filename].push(b.content), b.last)) {
                    var c = {blobUrl: q[b.filename].join(""), name: b.filename};
                    c.save = function () {
        ...
        function l(a, b) {
            c.send(JSON.stringify({eventName: a, data: b}))
        }
        ...
        var n, o, p = "receiver", q = {}, r = new a.EventEmitter, s = a.CONNECTING, t = {optional: [{VoiceActivityDetection: !1}]};
        return d(), r
    };
    var a = a || {};
    a.Sender = function (b, c) {
        function d() {
            function c(a) {
                if (a.candidate) {
                    ...
                    data.sdpMLineIndex = a.candidate.sdpMLineIndex;
                    data.candidate = a.candidate.candidate;
                    var b = {to: r, channelId: q, candidate: data, type: "offer"};
            ...
            function d(c) {
                f.setLocalDescription(new a.SessionDescription(c), function () {
                    ...
                    data.type = c.type;
                    data.sdp = c.sdp;
                    var a = {to: r, sdp: data, channelId: q, config: {stream: !!b.stream, data: b.data, attr: b.attr}};
            ...
            f.onicecandidate = c, b.stream && (u.stream = b.stream, f.addStream(b.stream.getStream())/*TODO 改动：这个位置仍然存在bug，IE下偶尔提示不存在getStream方法，应该是stream初始化不正常*/, u.emit("debug_log", {eventName: "add stream"})), b.data && (e(f), u.emit("debug_log", {eventName: "add datachannel"})), h(f), f.createOffer(d, o, w), u.receive = function (b) {
                f.setRemoteDescription(new a.SessionDescription(b), function () {
                    var data = {};
                    data.type = b.type;
                    data.sdp = b.sdp;
                    var c = {eventName: "receive answer", from: r, sdp: data};
                    v = a.CONNECTED, u.emit("debug_log", c)
                }, o)
            }, u.candidate = function (a) {
                f.addIceCandidate(a), u.emit("debug_log", {eventName: "add candidate"})
            }, u.close = function () {
        ...
        function i(b, c) {
            /*改动：这个状态监听循环在IE下会崩溃，估计是IE性能问题和插件的bug，不是必须监听，这里注释掉*/
            /*a.isMoz || b.getStats(function (a) {
                var b = [];
                a.result().forEach(function (a) {
                    var c = {};
        ...
        var q, r, s = "sender", t = !1, u = new a.EventEmitter, v = a.CONNECTING, w = {optional: [{VoiceActivityDetection: !1}]};
        return d(), u
    };
    var a = a || {};
    a.Session = function (b, c) {
        function d() {
            f.onopen = function () {
                l.token = b, h = (new Date).getTime(), f.sendMessage("__join", {t: h}), m = a.CONNECTED
            }, f.onmessage = function (a) {
                var b = JSON.parse(a.data), c = b.eventName, d = b.data;
                l.emit(c, d)
                ...
                f.send(JSON.stringify({eventName: a, data: b}))
            }
        }

        function e(a, b) {
            var c = atob(a), d = btoa(b);
            return c.replace(d, "")
        }

        var f, g, h, i = {}, j = {}, k = {}, l = new a.EventEmitter, m = a.DISCONNECTED;
        return l.connect = function () {
            ...
            var c = {iceServers: g};
            b && "object" == typeof b ? (b.data && (c.data = !0), b.attr && "object" == typeof b.attr && (c.attr = b.attr), b.stream && b.stream.local && (c.stream = b.stream)) : c = {data: !0};
            for (var d in k) {
                c.to = d;
    ...
    a.Stream = function (b) {
        ...
        function e() {
            function b(b, c) {
                /*该方法改动较大，主要是IE插件要求video标签必须已经处于文档流，这里将原框架中插入文档流操作提前*/
                var d = document.createElement("video");
                var e = "";
                e = -1 == b.indexOf("#") && -1 == b.indexOf(".") ? "#" + b : b;
                var f = document.querySelector(e);
                f.appendChild(d);/*改动：插入到文档流*/
               // var d = document.getElementById("video");
                d.setAttribute("width", c.width), d.setAttribute("height", c.height), d.setAttribute("autoplay", "autoplay"), d.pause(),
                     attachMediaStream(d,m)/*改动：显示视频流，使用IE插件中的兼容性方法*/,
                    /*d.src = a.URL.createObjectURL(m),*/
                m.getVideoTracks().length == [] && d.setAttribute("poster", j(c.width, c.height)),
                q.local && (d.volume = 0, d.muted = 0);

                return f ?  d : void q.emit("play-error", new a.Errors("play-error", 2, 0))
            }
            ...
            q.play = function (a, c) {
                ...
                s && (s = !1/*, q.player.remove()*//*改动：删除视频元素，IE下不是video标签，不能直接删除，可在页面中自己清空video所在容器*/, delete q.player)
            }, q.resize = function (a) {
                if (s) {
                    var b = k(a);
                ...
                if (s) {
                    var c = !0;
                    if (b.length <= 4 && b.length > 0)for (var d in b)!parseFloat(b[d]) && parseFloat(0 != b[d]) && (c = !1); else c = !1;
        ...
        function f() {
            a.extensionId ? chrome.runtime.sendMessage(a.extensionId, {getStream: !0}, function (b) {
                if (b) {
                    var c = b.streamId, d = {
                        audio: !1,
                        video: {
        ...
        function g() {
            var b = {audio: q.audio, video: {mandatory: {}, optional: []}};
        ...
        var m, n, o, p, q = new a.EventEmitter, r = !1, s = !1;
        return c(), q
    };
    var a = a || {};
